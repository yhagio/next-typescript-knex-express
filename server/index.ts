...
app.prepare().then(() => {
    const db = new DB(appConfig).getConnection();
    const errorHandler = ErrorHandler();

    const authService = new AuthService(appConfig);
    const userService = new UserService(
        new UserValidator(new UserDataAccess(db))
    );
    ...
    restRouter.route('/auth/signup').post(authHandler.signUp.bind(authHandler));
    restRouter.route('/auth/login').post(authHandler.login.bind(authHandler));
    restRouter.route('/auth/logout').get(authHandler.logout.bind(authHandler));
    restRouter
        .route('/users/:userId')
